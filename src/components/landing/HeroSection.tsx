    ...
    <div id="hero" className={`${styles.heroSection} relative min-h-screen flex items-center justify-center overflow-hidden`}>
      ...
      <div className={`relative z-10 text-center max-w-6xl mx-auto px-4 transition-all duration-1000 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
        ...
        <div className="mb-16">
          ...
          <h1 className={`${styles.heroTitle} ${isMobile ? 'text-3xl' : isDesktop ? 'text-7xl' : 'text-5xl'} font-black mb-8 tracking-tight`}>
          ...
          <div className="relative">
            <p className={`${styles.heroSubtitle} ${isMobile ? 'text-base' : isDesktop ? 'text-2xl' : 'text-xl'} text-gray-200 max-w-4xl mx-auto leading-relaxed font-light`}>
        ...
        <div className="relative inline-block group mb-16">
          ...
          >
            ...
            <span className="relative flex items-center justify-center gap-2">
              <span className={`${isMobile ? 'text-lg' : 'text-2xl'} animate-bounce`}>🚀</span>
              <span className="tracking-wide font-extrabold">{getStartedText}</span>
              <span className={`${isMobile ? 'text-sm' : 'text-lg'} animate-pulse`}>⚡</span>
            </span>
          </button>
        </div>
            ...
            <div key={index} className="group relative">
              ...
              <div className={`relative ${isMobile ? 'p-6' : 'p-8'}`}>
                ...
                <div className="relative mb-6">
                  ...
                  <div className={`relative ${isMobile ? 'text-4xl' : 'text-5xl'} mb-4 transform group-hover:scale-110 group-hover:rotate-12 transition-all duration-500`}>
                    {feature.icon}
                  </div>
                </div>

                {/* Feature Content */}
                <h3 className={`text-honey font-bold ${isMobile ? 'text-lg' : 'text-xl'} mb-3 group-hover:text-yellow-300 transition-colors duration-300`}>
                  {feature.title}
                </h3>
                <p className={`text-gray-300 ${isMobile ? 'text-xs' : 'text-sm'} leading-relaxed group-hover:text-gray-200 transition-colors duration-300`}>
                  {feature.desc}
                </p>

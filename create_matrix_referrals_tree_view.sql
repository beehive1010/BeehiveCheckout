-- 基于matrix_referrals表创建matrix_referrals_tree_view
-- 按照表中的顺序来排列
...
WITH matrix_positioned AS (
  -- 基于matrix_referrals表，为每个matrix_root按顺序分配位置
  SELECT 
...
matrix_with_layers AS (
  -- 根据位置序号计算layer和position
  SELECT 
    mp.*,
    -- 计算layer：1-3是layer1，4-12是layer2，13+是layer3...
    CASE 
      WHEN mp.matrix_position <= 3 THEN 1
      WHEN mp.matrix_position <= 12 THEN 2
      WHEN mp.matrix_position <= 39 THEN 3
      ELSE LEAST(CEIL((mp.matrix_position - 3) / 9.0) + 1, 19)
    END as layer,
    -- 计算position：L, M, R循环
    (ARRAY['L','M','R'])[(mp.matrix_position - 1) % 3 + 1] as position
  ...
  SELECT 
    ...
    CASE 
      ...
      WHEN mwl.layer >= 2 THEN (
        ...
        AND mwl_parent.matrix_position = ((mwl.matrix_position - 4) / 3)::int + 1
        LIMIT 1
      )
...
SELECT 
  ...
  -- 判断是否spillover
  CASE 
    WHEN mwp.referrer_wallet = mwp.matrix_root_wallet THEN false 
    ELSE true 
  END as is_spillover,
  true as is_activated,
  activation_time as child_activation_time,
  CASE 
    WHEN mwp.referrer_wallet = mwp.matrix_root_wallet THEN 'direct'
    ELSE 'spillover'
...
SELECT 
  ...
  0 as matrix_position, -- root的位置是0
  0 as referral_depth,
  0 as layer,
  'root' as position,

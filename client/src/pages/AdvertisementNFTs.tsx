...
export default function AdvertisementNFTs() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <Card className="bg-secondary border-border">
          <CardContent className="p-4 flex items-center gap-6">
            <div className="flex items-center gap-2">
              <IconWallet className="w-5 h-5 text-honey" />
              <span className="text-muted-foreground">Transferable:</span>
              <span className="text-honey font-semibold">{bccBalance?.transferable || 0} BCC</span>
            </div>
            <div className="flex items-center gap-2">
              <IconWallet className="w-5 h-5 text-blue-400" />
              <span className="text-muted-foreground">Restricted:</span>
              <span className="text-blue-400 font-semibold">{bccBalance?.restricted || 0} BCC</span>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      {/* Tab Navigation */}
      <div className="flex justify-center mb-8">
        <div className="bg-secondary/50 border border-honey/20 rounded-xl p-1 backdrop-blur-sm">
          <div className="flex">
            <button
              onClick={() => setActiveTab('marketplace')}
              className={`px-6 py-3 rounded-lg font-medium transition-all duration-300 ${
                activeTab === 'marketplace'
                  ? 'bg-honey text-black shadow-lg shadow-honey/25'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {isLoadingMyNFTs ? (
            ...
            <div className="col-span-full text-center py-12">
              ...
              <h3 className="text-xl font-semibold text-muted-foreground mb-2">No Advertisement NFTs</h3>
              <p className="text-muted-foreground">Claim some Advertisement NFTs from the marketplace to see them here</p>
            </div>
          ) : (
            myNFTs.map((claim) => (
              <Card key={claim.id} className="bg-secondary border-border glow-hover group">
                <div className="relative overflow-hidden rounded-t-lg">
                  <img
                  ...
                  <div className="absolute top-3 right-3">
                    <Badge className={claim.status === 'burned' ? 'bg-red-500/20 text-red-400 border-red-500/30' : 'bg-green-500/20 text-green-400 border-green-500/30'}>
                      {claim.status === 'burned' ? 'BURNED' : 'ACTIVE'}
                    </Badge>
                  </div>
                  <div className="absolute top-3 left-3">
                    <Badge className="bg-honey text-black">
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-3 mb-4">
                    <div className="flex justify-between items-center">
                      <span className="text-muted-foreground">BCC Locked:</span>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-blue-400">{claim.bucketUsed}</span>
                    </div>
                    {claim.activeCode && claim.status === 'claimed' && (
                      <div className="bg-secondary/50 rounded-lg p-3">
                        <div className="flex items-center gap-2 mb-1">
                          <IconCode className="w-4 h-4 text-honey" />
                          <span className="text-sm text-muted-foreground">Active Code:</span>
                        </div>
                        <code className="text-xs font-mono text-honey break-all">{claim.activeCode}</code>
                      </div>
                    )}
                  </div>
                  ...
                  {claim.status === 'claimed' ? (
                    <Dialog>
                      <DialogTrigger asChild>
                        <Button 
                          ...
                          data-testid={`button-burn-${claim.id}`}
                        >
                          <FireIcon className="w-4 h-4 mr-2" />
                          Burn for Service Code
                        </Button>
                      </DialogTrigger>
                      <DialogContent className="bg-background border-border">
                        <DialogHeader>
                          <DialogTitle className="text-red-400">Burn Advertisement NFT</DialogTitle>
                          <DialogDescription className="text-muted-foreground">
                            This will permanently burn your NFT and release the service code. This action cannot be undone.
                          </DialogDescription>
                        </DialogHeader>
                        <div className="space-y-4">
                          <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4">
                            <h4 className="font-semibold text-red-400 mb-2">⚠️ Warning</h4>
                            <p className="text-sm text-muted-foreground">
                              Burning this NFT will destroy it forever and release the service code for {claim.nft.serviceName}. 
                              The {claim.bccAmountLocked} BCC tokens will also be permanently burned.
                            </p>
                          </div>
                          <Button
                            onClick={() => burnNFTMutation.mutate(claim.id)}
                            disabled={burnNFTMutation.isPending}
                            className="w-full bg-red-600 hover:bg-red-700 text-white"
                            data-testid={`button-confirm-burn-${claim.id}`}
                          >
                            {burnNFTMutation.isPending ? 'Burning...' : 'Confirm Burn'}
                          </Button>
                        </div>
                      </DialogContent>
                    </Dialog>
                    ...
                    <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-3">
                      <div className="flex items-center gap-2 text-green-400">
                        ...
                        <span className="text-sm font-semibold">Service Activated</span>
                      </div>
                      <p className="text-xs text-muted-foreground mt-1">
                        This NFT has been burned and the service code was released
                      </p>
                    </div>
                  )}
                </CardContent>

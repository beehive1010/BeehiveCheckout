...
**关键发现 - 每层都有原生直推占位**:
```
Layer | 原生直推 | 滑落成员 | 总计 | 原生占比 | 占位说明
------|---------|---------|------|---------|----------
  1   |    3    |    0    |   3  |  100%   | 纯Layer1直推
  2   |    9    |    6    |  15  |   60%   | Layer2直推 + Layer1滑落
  3   |   12    |   20    |  32  |  37.5%  | Layer3直推 + 上层滑落
  4   |    6    |    0    |   6  |  100%   | 纯Layer4直推
  5   |   22    |    0    |  22  |  100%   | 纯Layer5直推
  6+  |   各1    |    0    | 各1  |  100%   | 各层原生直推
```

**核心机制**: 
- ✅ **每层都有原生直推占位**：Layer N的成员优先占Layer N位置
- ✅ **激活时间决定占位顺序**：同层内按激活时间排序
- ✅ **滑落只发生在前3层**：Layer1-3有容量溢出，其他层都是原生直推
- ✅ **第4层及以后**：完全匹配referrals_tree_view的depth层级
- ✅ 所有递归的直推和滑落都在 `matrix_referrals_tree_view` 中按时间序号正确显示

...
interface CleanupRequest {
  action: 'cleanup-expired-users' | 'cleanup-single-user' | 'get-cleanup-candidates';
    ...
    switch (action) {
      ...
      case 'cleanup-single-user':
        if (!requestData.walletAddress) {
          return new Response(
            JSON.stringify({ error: 'Wallet address required for single user cleanup' }),
...
async function handleGetCleanupCandidates(supabase: any) {
  try {
    const now = new Date()
    ...
    const { data: candidates } = await supabase
      ...
      `)
      .eq('members.is_activated', false)
      .neq('referrer_wallet', ROOT_WALLET) // Don't cleanup users with root referrer
    ...
    if (candidates) {
      for (const user of candidates) {
        ...
        const hasActiveCountdown = user.countdown_timers?.some((ct: any) => 
          ct.is_active && new Date(ct.end_time) > now
        )
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to get cleanup candidates', details: error.message }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
}

async function handleCleanupExpiredUsers(supabase: any, dryRun: boolean) {
  try {
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: dryRun 
          ? `Dry run: Would clean up ${cleanedCount} users` 
          : `Successfully cleaned up ${cleanedCount} users`,
...
async function handleCleanupSingleUser(supabase: any, walletAddress: string, dryRun: boolean) {
  try {
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to cleanup user', details: error.message }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
}

async function cleanupSingleUserInternal(supabase: any, walletAddress: string, dryRun: boolean) {
  try {
    const ROOT_WALLET = '0x0000000000000000000000000000000000000001'
    ...
    if (!userData) {
      return { success: false, message: 'User not found', walletAddress }
    }
    ...
    const now = new Date()
    const hasActiveCountdown = userData.countdown_timers?.some((ct: any) => 
      ct.is_active && new Date(ct.end_time) > now
    )

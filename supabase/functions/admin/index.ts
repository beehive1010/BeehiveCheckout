...
interface AdminRequest {
  action: 'get-settings' | 'toggle-global-pending' | 'set-member-pending' | 'clear-member-pending' | 'get-pending-members' | 'get-admin-actions' | 'update-setting';
  targetWallet?: string;
  pendingHours?: number;
  enabled?: boolean;
  reason?: string;
...
serve(async (req) => {
  ...
  try {
    ...
    const { action, ...requestData }: AdminRequest = await req.json()
...
async function handleGetSettings(supabase: any, walletAddress: string) {
  try {
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to fetch admin settings' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
}

async function handleToggleGlobalPending(supabase: any, walletAddress: string, data: any) {
  try {
    ...
    const { data: result, error } = await supabase
      .rpc('toggle_activation_pending_global', {
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to toggle global pending setting' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
}

async function handleSetMemberPending(supabase: any, walletAddress: string, data: any) {
  try {
    ...
    const { data: result, error } = await supabase
      .rpc('set_member_activation_pending', {
        p_admin_wallet: walletAddress,
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to set member activation pending' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
}

async function handleClearMemberPending(supabase: any, walletAddress: string, data: any) {
  try {
    ...
    const { data: result, error } = await supabase
      .rpc('clear_member_activation_pending', {
        p_admin_wallet: walletAddress,
        p_target_wallet: targetWallet.toLowerCase(),
        p_reason: reason
      })
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to clear member activation pending' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
}

async function handleGetPendingMembers(supabase: any, walletAddress: string) {
  try {
    const { data: pendingMembers, error } = await supabase
      .rpc('get_pending_activations', {
        p_admin_wallet: walletAddress
      })
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to fetch pending members' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
}

async function handleGetAdminActions(supabase: any, walletAddress: string, data: any) {
  try {
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to fetch admin actions' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
}

async function handleUpdateSetting(supabase: any, walletAddress: string, data: any) {
  try {

...
interface BalanceRequest {
  action: 'get-balance' | 'get-transactions' | 'transfer-bcc' | 'spend-bcc' | 'get-earning-history';
  amount?: number;
...
serve(async (req) => {
  ...
  try {
    ...
    const { action, ...requestData }: BalanceRequest = await req.json()
...
async function handleGetBalance(supabase: any, walletAddress: string) {
  try {
    ...
    const { data: recentPurchases, error: purchaseError } = await supabase
      ...
      .select('amount_bcc, status, created_at')
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to fetch balance data' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
}
...
async function handleTransferBcc(supabase: any, walletAddress: string, data: any) {
  try {
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to transfer BCC tokens' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
}
...
async function handleSpendBcc(supabase: any, walletAddress: string, data: any) {
  try {
    ...
    if (itemType === 'nft') {
      // Handle NFT purchase
      const { data: nftResult, error: nftError } = await supabase
        .rpc('purchase_nft_with_bcc', {
      ...
      result = nftResult
      purchaseRecord = { type: 'nft', nftId: itemId, nftType: data.nftType || 'merchant' }

    } else if (itemType === 'course') {
      // Handle course purchase
      const { data: courseResult, error: courseError } = await supabase
        .rpc('purchase_course_with_bcc', {
      ...
      result = courseResult
      purchaseRecord = { type: 'course', courseId: itemId }
      ...
      result = spendResult
      purchaseRecord = { type: itemType, itemId: itemId }
    }
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to spend BCC tokens' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
}
...
async function handleGetEarningHistory(supabase: any, walletAddress: string, data: any) {
  try {
    ...
    const { data: bccHistory, error: bccError } = await supabase
      .from('bcc_purchase_orders')
      .select(`
        order_id,
        amount_bcc,
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to fetch earning history' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
}
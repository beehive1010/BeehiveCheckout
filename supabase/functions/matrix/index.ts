...
interface MatrixRequest {
  action: 'get-matrix' | 'place-member' | 'get-downline' | 'get-upline' | 'get-matrix-stats';
  rootWallet?: string;
  memberWallet?: string;
  placerWallet?: string;
...
serve(async (req) => {
  ...
  try {
    ...
    const { action, ...requestData }: MatrixRequest = await req.json()
...
async function handleGetMatrix(supabase: any, walletAddress: string, data: any) {
  try {
    ...
    const response = {
      ...
      matrix: {
        ...
        stats: {
          totalMembers,
          activeMembers,
          layers: Math.max(...(matrixData?.map(m => m.layer) || [0]))
        }
      }
    }
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to fetch matrix data' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
}

async function handlePlaceMember(supabase: any, walletAddress: string, data: any) {
  try {
    ...
    const { data: result, error: placementError } = await supabase
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to place member in matrix' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
}

async function handleGetDownline(supabase: any, walletAddress: string, data: any) {
  try {
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to fetch downline data' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
}

async function handleGetUpline(supabase: any, walletAddress: string, data: any) {
  try {
    // Find all upline members (where current user is in their matrix)
    const { data: uplineData, error } = await supabase
      .from('referrals')
      .select(`
        root_wallet,
        layer,
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to fetch upline data' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
}

async function handleGetMatrixStats(supabase: any, walletAddress: string) {
  try {
    ...
    const { data: referralCount, error: countError } = await supabase
      .from('referrals')
      .select('member_wallet', { count: 'exact' })
      .eq('root_wallet', walletAddress)
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to fetch matrix statistics' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
}
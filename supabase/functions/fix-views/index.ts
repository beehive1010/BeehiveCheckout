...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'fix-referrer-stats') {
      // Simple referrer_stats view without problematic table references
      const createViewSQL = `
      ...
      const { error } = await supabaseClient.rpc('exec', { sql: createViewSQL })
      ...
      return new Response(
        JSON.stringify({ 
          success: true, 
          message: 'Simple referrer_stats view created successfully' 
        }),
        { 

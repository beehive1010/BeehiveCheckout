  ...
  try {
    ...
    try {
      ...
      const { data: existingLayerReward, error: checkError } = await supabase
        ...
        .select('id, reward_status, reward_amount')
        .eq('rewarded_member_wallet', userData.wallet_address)
    ...
    try {
      ...
      if (thirdwebClientId && memberRecord) {
        ...
        const verificationQueries = await Promise.allSettled([
          ...
          supabase.from('matrix_layer_rewards').select('id').eq('rewarded_member_wallet', userData.wallet_address).eq('matrix_layer', level).maybeSingle()
        ]);

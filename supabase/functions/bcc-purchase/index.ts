...
interface BccPurchaseRequest {
  action: 'get-config' | 'create-purchase' | 'confirm-payment' | 'get-history' | 'get-pending';
  amountUSDC?: number;
  network?: string;
...
serve(async (req) => {
  ...
  try {
    ...
    const { action, ...requestData }: BccPurchaseRequest = await req.json()
...
async function handleCreatePurchase(supabase: any, walletAddress: string, data: any) {
  try {
...
async function handleConfirmPayment(supabase: any, walletAddress: string, data: any) {
  try {
...
async function handleGetHistory(supabase: any, walletAddress: string, data: any) {
  try {
...
async function handleGetPending(supabase: any, walletAddress: string) {
  try {

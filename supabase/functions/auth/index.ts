...
interface AuthRequest {
  action: 'login' | 'register' | 'get-user' | 'claim-nft-token-1' | 'create-referral-link' | 'process-referral-link' | 'get-countdowns' | 'create-countdown';
...
serve(async (req) => {
  ...
  try {
    ...
    const { action, ...requestData }: AuthRequest = await req.json()
  ...
  try {
    ...
    const { data: sessionResult, error: sessionError } = await supabase
      .rpc('create_wallet_session', {
        ...
        p_signature: data.signature || 'demo_signature',
        p_message: data.message || 'Login to Beehive Platform'
      })
